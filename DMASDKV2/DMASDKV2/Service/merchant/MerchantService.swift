...
open class MerchantService: NSObject {
    ...
    public func deploy(privateKey:String,gasPrice:String,gasLimit:String,name:String,symbol:String,metadata:String,token20:String) -> ContractResult {
        let eth = EthService()
        eth.url = url
        ...
        case .success(let resp):
            ...
            if Double(b)! > 0{
                ...
                case .success(let value):
                    ...
                    let platformResult = platform.setupDeploy(privateKey: privateKey, token721: assetAddress, token20: token20, platformAddress: platformWallet,firstExpense: firstExpenses,secondExpense: secondExpenses ,gasLimit: gasLimit, gasPrice: gasPrice)
    ...
    ///   - owner: 资产拥有者地址
    /// - Returns: return value description
    public  func createOrder(platAddress:String,token20Address : String,privateKey:String,gasPrice:String = "",gasLimit:String = "",tokenIds:Array<Any>,sumPrice:String,owner:String,getGasFee : Bool = false) -> ContractResult {
        let platform = PlatformContract(url: url)
        ...
        let tokencontact = TokenContract(url: url)
