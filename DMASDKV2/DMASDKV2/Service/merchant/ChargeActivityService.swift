...
public class ChargeActivityService : NSObject{
    ...
    public  func createOrder(platAddress:String,token20Address : String,privateKey:String,gasPrice:String = "",gasLimit:String = "",tokenIds : Array<Any>,sumPrice:String,owner:String,getGasFee : Bool = false) -> ContractResult {
        ...
        if !getGasFee{
            let result = platform.transferWithArray(privateKey: privateKey, contractAddress: platAddress, owner: owner, tokenArr: tokenIds, value: sumPrice, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : true)
            let isError = limIsEmpty(gasLimit: &gasLimit, gasPrice: &gasPrice, getGasFee: &getGasFee, result: result)
            if let result = isError{
                return result
            }
        ...
        case .success(let value):
            ...
            let platform = ChargeActivityContract(url: urlStr)
            let platformResult = platform.transferWithArray(privateKey: privateKey, contractAddress: platAddress, owner: owner, tokenArr: tokenIds, value: sumPrice, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : getGasFee)
            switch platformResult{
