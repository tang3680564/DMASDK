...
public class PledgeActivityService : NSObject{
    ...
    public func deploy(privateKey:String,gasPrice:String = "",gasLimit:String = "",getGasFee:Bool = false) -> ContractResult {
        ...
        case .success(let resp):
            ...
            if Double(b)! > 0{
                let platformResult = platform.setupDeploy(privateKey: privateKey, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : getGasFee)
    ...
    /// - Returns:
    public func onSales(contractAddress:String,platformAddress:String,privateKey:String,gasLimit:String = "",gasPrice:String = "",owner:String,endTime:String,tokenIds:Array<Any>,price:String,getGasFee : Bool = false,canNext : Bool = false) -> ContractResult {
        var canNext = canNext
        var gasLimit = gasLimit
        var gasPrice = gasPrice
        if getGasFee{
            ...
            return onSalesInFor(contractAddress: contractAddress, platformAddress: platformAddress, privateKey: privateKey, gasLimit: gasLimit, gasPrice: gasPrice, owner: owner, tokenIds: tokenIds, price: price, endTime: endTime, getGasFees: true,canNext: canNext)
        }
        limAndPriceIsEmpty(gasLimit: &gasLimit, gasPrice: &gasPrice)
        let asset = AssetManagement(url: urlStr)
        //
        ...
        case .success(let value):
            ...
            if canNext{
                if gasValue.doubleValue < NSDecimalNumber(string: banlance).doubleValue{
                    return onSales(contractAddress: contractAddress,platformAddress: platformAddress, privateKey: privateKey, gasLimit: userLimt.stringValue, gasPrice: gasPrice, owner: owner, endTime: endTime, tokenIds: tokenIds, price: price)
                }else{
                    return ContractResult.failure(error: DMASDKError.BALANCE_UNDERFINANCED.getCodeAndMsg())
                }
            }
    ...
    ///   - getGasFee: 估算这次操作所需要的gasfee , true : 进行估算,不进行这次操作, false : 不进行估算,进行这次操作
    /// - Returns: return value description
    public  func createOrder(assetAddress:String,platAddress:String,privateKey:String,gasPrice:String = "",gasLimit:String = "",tokenId:String,sumPrice:String,owner:String,getGasFee : Bool = false) -> ContractResult {
        let platform = PledgeActivityContract(url: urlStr)
        ...
        var getGasFee = getGasFee
        if !getGasFee{
            let result = platform.transfer(privateKey: privateKey, assetAddress: assetAddress, contractAddress: platAddress, tokenId: tokenId, weiValue: sumPrice, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : true)
            let isError = limIsEmpty(gasLimit: &gasLimit, gasPrice: &gasPrice, getGasFee: &getGasFee, result: result)
            if let result = isError{
                return result
            }
        ...
        let platformResult = platform.transfer(privateKey: privateKey, assetAddress: assetAddress, contractAddress: platAddress, tokenId: tokenId, weiValue: sumPrice, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : getGasFee)
        switch platformResult{
    ...
    public func getApproveInfo(assetAddress : String,contractAddress : String,tokenId : String) -> ContractResult{
        let platform = PledgeActivityContract(url: urlStr)
        return platform.getApproveinfo(assetAddress:assetAddress,contractAddress: contractAddress, tokenId: tokenId)
    }
    ...
    ///   - getGasFee: 估算这次操作所需要的gasfee , true : 进行估算,不进行这次操作, false : 不进行估算,进行这次操作
    /// - Returns: hash
    public func refund(privateKey : String,assetAddress:String,contractAddress : String,tokenId : String,gasLimit : String = "",gasPrice : String = "",getGasFee : Bool = false) -> ContractResult{
        let platform = PledgeActivityContract(url: urlStr)
        var gasLimit = gasLimit
        var gasPrice = gasPrice
        var getGasFee = getGasFee
        if !getGasFee{
            let result = platform.refund(privateKey: privateKey, assetAddress: assetAddress, contractAddress: contractAddress, tokenId: tokenId, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : true)
            let isError = limIsEmpty(gasLimit: &gasLimit, gasPrice: &gasPrice, getGasFee: &getGasFee, result: result)
            if let result = isError{
                return result
            }
        ...
        return platform.refund(privateKey: privateKey, assetAddress: assetAddress, contractAddress: contractAddress, tokenId: tokenId, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : getGasFee)
    }
    ...
    ///   - getGasFee: 估算这次操作所需要的gasfee , true : 进行估算,不进行这次操作, false : 不进行估算,进行这次操作
    /// - Returns: hash
    public func verify(privateKey : String,assetAddress:String,contractAddress : String,tokenId : String,owner : String,gasLimit : String = "", gasPrice : String = "",getGasFee : Bool = false ) -> ContractResult{
        let platform = PledgeActivityContract(url: urlStr)
        var gasLimit = gasLimit
        var gasPrice = gasPrice
        var getGasFee = getGasFee
        if !getGasFee{
            let result = platform.verify(privateKey: privateKey, assetAddress: assetAddress, contractAddress: contractAddress, tokenId: tokenId, owner: owner, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : true)
            let isError = limIsEmpty(gasLimit: &gasLimit, gasPrice: &gasPrice, getGasFee: &getGasFee, result: result)
            if let result = isError{
                return result
            }
        ...
        return platform.verify(privateKey: privateKey, assetAddress: assetAddress, contractAddress: contractAddress, tokenId: tokenId, owner: owner, gasLimit: gasLimit, gasPrice: gasPrice,getGasFee : getGasFee)
    }
